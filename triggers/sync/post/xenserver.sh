...
dstdir=/tftpboot
for profile in $( grep -l xen602.ks $libdir/config/profiles.d/* ); do
    json=${profile##*/}
    name=${json%.json}
    [[ -d $dstdir/images/$name ]] && \
    rsync -av $srcdir/ks_mirror/$name/{install.img,boot/vmlinuz,boot/isolinux/mboot.c32} \
        $dstdir/images/$name/
    cp -f /pxe/xen56sp2source/boot/isolinux/mboot.c32 $dstdir/images/$name
done
